...
def main() -> None:
    ...
    set_seed(args.seed)
    rows = read_jsonl(data_cfg["forget_file"]) + read_jsonl(data_cfg["retain_file"])
    ...
    fine_tune_lm(model, tokenizer, rows_to_lm_texts(rows), train_cfg, args.output, method="full_finetune")
