...
HLC_CONFIG="${HLC_CONFIG:-configs/method/hlc_sg_k2_gpt2_lora_smoke.yaml}"
HLC_LABEL="${HLC_LABEL:-hlc_sg_k2}"
OUTPUT_ROOT="${OUTPUT_ROOT:-runs/survival/gpt2_hlc_k2_smoke}"
REPORT_DIR="${REPORT_DIR:-runs/reports/gpt2_hlc_k2_smoke}"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      cat <<'USAGE'
Usage: scripts/reproduce_gpt2_hlc_k2_smoke.sh [--overwrite] [--skip-tests]

Runs a stronger GPT-2 HLC candidate after the base GPT-2 LoRA smoke lane.
Requires the outputs from scripts/reproduce_gpt2_lora_smoke.sh.

Outputs:
  checkpoints/unlearned/hlc_sg_k2_gpt2_lora_tofu50_seed0
  runs/survival/gpt2_hlc_k2_smoke/
...
require_path() {
  if [[ ! -e "$1" ]]; then
    ...
    echo "Run scripts/reproduce_gpt2_lora_smoke.sh --skip-tests first." >&2
    exit 1
  fi
}
...
require_path "checkpoints/gpt2_lora_full_seed${SEED}"
...
K2_CHECKPOINT="checkpoints/unlearned/${HLC_LABEL}_gpt2_lora_tofu50_seed${SEED}"
if [[ "$OVERWRITE" -eq 1 || ! -f "${K2_CHECKPOINT}/run_metadata.json" ]]; then
...
for pool in "${POOLS[@]}"; do
  ...
  maybe_run_survival \
    ...
    "$pool" \
    "${OUTPUT_ROOT}/${HLC_LABEL}_gpt2_lora_seed${SEED}_${label}"
  COMPARE_INPUTS+=("${OUTPUT_ROOT}/${HLC_LABEL}_gpt2_lora_seed${SEED}_${label}/survival_curve.csv")
  COMPARE_INPUTS+=("runs/survival/gpt2_lora_smoke/hlc_sg_k1_gpt2_lora_seed${SEED}_${label}/survival_curve.csv")
  COMPARE_INPUTS+=("runs/survival/gpt2_lora_smoke/npo_strong_gpt2_lora_seed${SEED}_${label}/survival_curve.csv")
done
...
echo
echo "GPT-2 HLC K2 smoke diagnostic complete."
