...
def build_config(template: dict[str, object], steps: int, forget_weight: float, beta: float) -> dict[str, object]:
    cfg = dict(template)
    cfg["method_name"] = "syntactic_diversification_sweep"
    cfg["steps"] = int(steps)
    cfg["forget_weight"] = float(forget_weight)
    cfg["beta"] = float(beta)
    cfg["use_prompt_variants"] = True
    return cfg
...
def main() -> None:
    ...
    parser.add_argument("--template_config", default="configs/method/syntactic_diversification_fullft_smoke.yaml")
    ...
    parser.add_argument("--train_prompt_variants", default="data/prompts/forget_syntactic_variants.jsonl")
    parser.add_argument("--eval_prompt_variants", default="data/prompts/forget_prompt_variants.jsonl")
    ...
    parser.add_argument("--config_dir", default="configs/method/sweeps/syntactic_immediate_matched")
    parser.add_argument("--checkpoint_root", default="checkpoints/unlearned/syntactic_diversification_sweep")
    parser.add_argument("--eval_root", default="runs/survival/syntactic_diversification_sweep_t0")
        ...
        for forget_weight in forget_weights:
            for beta in betas:
                ...
                for seed in seeds:
                    ...
                    method_label = f"syntactic_diversification_sweep_seed{seed}_{label}"
